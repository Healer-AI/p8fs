...
class MobileAuthenticationService:
    ...
    async def register_device(
        ...
        4. Create device with UNVERIFIED status
        ...
        Returns:
            Device object with verification challenge
            
        Raises:
            MobileAuthenticationError: Invalid key or limit exceeded
        """
        # In simple tenant system, we don't track devices separately
        # Device limits are not enforced - each tenant can have multiple access methods
        ...
        # In simple tenant system, we can check if public key is already registered
        # by checking if a tenant with this public key already exists
        # For now, we'll skip this check since we don't have reverse lookup by public key
        # In the future, this could be implemented by storing device info in tenant metadata
        ...
        challenge_data = {
            ...
            "expires_at": (
                datetime.utcnow() + timedelta(seconds=self.challenge_ttl)
            ).isoformat(),
            "attempts": 0
        }
        
        # Create device with UNVERIFIED status
        # Reference: p8fs-auth/docs/authentication-flows.md - "Server creates pending registration"
        device = Device(
            device_id=str(secrets.token_urlsafe(16)),
            public_key=public_key_base64,
            email=email,
            device_name=device_name or "Unknown Device",
            trust_level=DeviceTrustLevel.UNVERIFIED,
            challenge_data=challenge_data,
            metadata=device_info or {}  # Store full device_info including potential IMEI
        )
        
        # Save device
        await self.auth_repository.create_device(device)
        
        # Log device creation with key details
        has_imei = bool(device_info and device_info.get("imei"))
        logger.info(f"Device created: {device.device_id} email={device.email} trust={device.trust_level.value} has_imei={has_imei}")
        
        # In production, send verification code via email
        # For now, include in response for testing
    ...
    async def authenticate_with_signature(
        ...
        # Issue proper JWT tokens using the JWT service
        if self.jwt_manager:
            # Create proper JWT access token for the device's user
            access_token = await self.jwt_manager.create_access_token(
                user_id=device.device_id,  # Use device_id as user identifier for mobile auth
