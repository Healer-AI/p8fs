...
class LLMApiRequest(BaseModel):
    ...
    def prepare_request_data(
        self, params: dict[str, Any], source_scheme: str
    ) -> dict[str, Any]:
        """
    ...
    def prepare_openai_request(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if not token:
            raise ValueError(
                "OpenAI API token is required but not found. "
    ...
    def prepare_anthropic_request(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        # Stream value comes from the request object, don't override it
    ...
    def prepare_google_request(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if not api_key:
            raise ValueError(
                "Google API key is required but not found. "
...
class OpenAIRequest(LLMApiRequest):
    ...
    max_completion_tokens: int | None = None  # GPT-5 requires this instead of max_tokens
    ...
    def to_anthropic_format(self) -> dict[str, Any]:
        ...
        Note: Anthropic does not allow both temperature and top_p to be specified.
        We only send temperature to Anthropic.
        Anthropic doesn't have native JSON mode - we add JSON instruction to system prompt if requested.
        """
        result = {
            "model": self.model,
            "max_tokens": self.max_tokens,
            "temperature": self.temperature,
            # Don't include top_p - Anthropic doesn't allow both temperature and top_p
            "stream": self.stream,
        }
        ...
        # Add JSON mode instruction if response_format is set
    ...
    def to_google_format(self) -> dict[str, Any]:
        """Convert OpenAI format to Google format with JSON mode support"""
        generation_config = {
            "temperature": self.temperature,
            "topP": self.top_p,
        ...
        # Add JSON mode if response_format is set
        if self.response_format and self.response_format.get("type") == "json_object":
    ...
    def to_openai_format(self) -> dict[str, Any]:
        """Return self as dict, filtering out unsupported parameters for GPT-5"""
        result = self.model_dump(exclude_none=True)
        
        # Filter out unsupported parameters for GPT-5 models
        model_name = result.get("model", "")
        is_gpt5 = model_name.startswith("gpt-5") or model_name == "gpt-5"
        
        if is_gpt5:
            # Remove parameters that GPT-5 doesn't support
            unsupported_params = ["temperature", "top_p", "stream", "max_tokens"]
...
class AnthropicRequest(LLMApiRequest):
    ...
    def to_openai_format(self) -> dict[str, Any]:
        ...
        if self.system:
            openai_messages.append({"role": "system", "content": self.system})
...
class GoogleRequest(LLMApiRequest):
    ...
    messages: list[dict[str, Any]] | None = (
        None  # Keep the messages field from LLMApiRequest
    )
    contents: list[dict[str, Any]] | None = None  # Google-specific contents field
    ...
    def check_messages(cls, values):
        ...
        if (
            "messages" in values
            and values["messages"]
            and ("contents" not in values or not values["contents"])
        ):
            contents = []
    ...
    def to_google_format(self) -> dict[str, Any]:
        ...
        # Ensure generationConfig is properly formatted
        if not self.generationConfig:
            result["generationConfig"] = {}

        # Move temperature, max_tokens, top_p to generationConfig
...
class StreamDelta(BaseModel):
    ...
    def to_sse_event(self) -> str:
        ...
        data = json.dumps(self.to_openai_format(),default=str)
        return f"data: {data}\n\n"
...
class OpenAIStreamDelta(StreamDelta):
    ...
    def to_anthropic_format(self) -> dict[str, Any]:
        ...
        # Default empty delta
        return {"type": "content_block_delta", "delta": {}}
...
class GoogleStreamDelta(StreamDelta):
    ...
    def to_openai_format(self) -> dict[str, Any]:
        ...
        result["choices"].append(
            {"index": 0, "delta": delta, "finish_reason": candidate.get("finishReason")}
        )

        # Add usage if present
        if self.usageMetadata:
            # Handle both simple and detailed token formats
            prompt_tokens = self.usageMetadata.get("promptTokenCount", 0)
            candidates_tokens = self.usageMetadata.get("candidatesTokenCount", 0)
            total_tokens = self.usageMetadata.get("totalTokenCount", 0)
            
            # Handle detailed token format (list of modality-specific counts)
            if "promptTokensDetails" in self.usageMetadata and isinstance(self.usageMetadata["promptTokensDetails"], list):
                prompt_tokens = sum(detail.get("tokenCount", 0) for detail in self.usageMetadata["promptTokensDetails"])
            
            if "candidatesTokensDetails" in self.usageMetadata and isinstance(self.usageMetadata["candidatesTokensDetails"], list):
                candidates_tokens = sum(detail.get("tokenCount", 0) for detail in self.usageMetadata["candidatesTokensDetails"])
            
            result["usage"] = {
                "prompt_tokens": prompt_tokens,
                "completion_tokens": candidates_tokens,
                "total_tokens": total_tokens or (prompt_tokens + candidates_tokens),
            }
...
class AnthropicStreamDelta(StreamDelta):
                ...
                result["choices"][0]["delta"]["tool_calls"] = [
                    {
                        ...
                        "function": {"arguments": self.delta.get("partial_json", "")},
                    }
                ]
    ...
    def to_content(self) -> str:
        ...
        if (
            ...
            and self.delta
            and self.delta.get("type") == "text_delta"
        ):
...
class OpenAIResponse(LLMResponse):
    ...
    def to_google_format(self) -> dict[str, Any]:
        ...
        if "content" in message and message["content"]:
            candidate["content"]["parts"].append({"text": message["content"]})
    ...
    def to_openai_format(self) -> dict[str, Any]:
        ...
        result = {
            ...
            "choices": [
                {
                    "index": 0,
                    "message": {"role": self.role, "content": text_content},
                ...
                tool_calls.append(
                    {
                        ...
                        "function": {
                            "name": func_call.get("name", ""),
                            "arguments": json.dumps(func_call.get("args", {}),default=str),
                        },
                    }
                )
...
class MessageStack:
    """
    Stack of messages for LLM agent sessions.

    This class manages conversation history and formats messages for
    agentic workflows in P8FS. It integrates with AbstractModel to
    build system prompts from model metadata.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            data: Initial data context for the conversation
        """
    ...
    def __iter__(self):
        """Iterate over messages in the stack."""
        for d in self.data:
            yield d

    def add(self, data: dict[str, Any] | list[dict[str, Any]], **kwargs):
        """
        Add messages to the stack.

        Args:
            data: Message(s) to add (dict or list of dicts)
    ...
    def build_message_stack(
        cls,
        abstracted_model: Any,  # Would be AbstractModel but avoiding import
        question: str,
        data: list[dict[str, Any]] | None = None,
        ...
        **kwargs,
        ...
        """
        Build a message stack from a model prompt and question.

        This creates a conversation context from an AbstractModel's metadata,
        including its description, schema, and registered functions.

        Args:
            abstracted_model: Model providing system prompt via description
            question: The user's question
            data: Initial data to include in context
            use_full_description: Whether to include full schema/functions
            user_memory: User context/memory to inject
            schema_format: Format for structured schema output ('yaml', 'json', etc.)
            **kwargs: Additional options like system_prompt_preamble

        Returns:
            Configured MessageStack instance
        """
        _data = []

        # Add initial data and user memory as context
        if data or user_memory:
            if data:
                _data.append({"role": "user", "content": json.dumps(data, default=str)})

            if user_memory:
                _data.append(
                    {"role": "user", "content": json.dumps(user_memory, default=str)}
                )

        # Build system prompt from model description
        generalized_prompt_preamble = kwargs.get("system_prompt_preamble", "")
        model_description = abstracted_model.get_model_description(
            use_full_description=use_full_description,
    ...
    def to_messages(self) -> list[dict[str, Any]]:
        """
        Convert the stack to OpenAI-style messages format.
