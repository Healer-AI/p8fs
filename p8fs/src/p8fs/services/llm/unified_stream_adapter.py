...
The adapter integrates with existing p8fs models and is designed to be testable
and maintainable with clear separation of concerns between parsing, buffering,
and format conversion.

Currently supports:
- OpenAI GPT models (pass-through with tool call buffering)
- Anthropic Claude models (conversion to OpenAI format)
- Google Gemini models (conversion to OpenAI format)

Based on the original UnifiedStreamAdapter but adapted to work with existing
p8fs model classes and architecture.
"""
...
class StreamEventType(Enum):
    ...
    USAGE = "usage"
    DONE = "done"
    STOP = "stop"  # Indicates finish_reason: "stop" - the final response
    ERROR = "error"
    ...
    def update_tool_call_args(self, index: int, args_delta: str) -> None:
        ...
        if index in self.tool_calls:
            if "function" in self.tool_calls[index]:
                if "arguments" not in self.tool_calls[index]["function"]:
...
class UnifiedStreamAdapter:
    ...
    1. Format conversion between providers (OpenAI, Anthropic, Google)
    ...
    - Google → OpenAI (full format conversion)
    """

    def __init__(self, source_scheme: str = "openai", target_scheme: str = "openai"):
        if source_scheme not in ["openai", "anthropic", "google"]:
            raise ValueError(f"Unsupported source scheme: {source_scheme}")
        if target_scheme not in ["openai", "anthropic", "google"]:
    ...
    def convert_to_canonical_format(self, chunk: Dict[str, Any]) -> Dict[str, Any]:
        """
        Convert a chunk from source format to OpenAI canonical format using p8fs models.
        ...
        if self.source_scheme == "openai":
            ...
            # Validate and return as-is, using OpenAIStreamDelta for validation
            try:
                delta = OpenAIStreamDelta(**chunk)
                return delta.to_openai_format()
            except Exception as e:
                logger.warning(f"Invalid OpenAI chunk: {e}, returning as-is")
                return chunk
        elif self.source_scheme == "anthropic":
            try:
                delta = AnthropicStreamDelta(**chunk)
                return delta.to_openai_format()
            except Exception as e:
                logger.error(f"Failed to convert Anthropic chunk: {e}")
                return chunk
        elif self.source_scheme == "google":
            try:
                delta = GoogleStreamDelta(**chunk)
                return delta.to_openai_format()
            except Exception as e:
                logger.error(f"Failed to convert Google chunk: {e}")
                return chunk
        else:
    ...
    def convert_to_target_format(self, chunk: Dict[str, Any]) -> Dict[str, Any]:
        """
        Convert a chunk from OpenAI format to target format using p8fs models.
        ...
        try:
            # Create OpenAI delta object and convert to target format
            delta = OpenAIStreamDelta(**chunk)
            if self.target_scheme == "anthropic":
                return delta.to_anthropic_format()
            elif self.target_scheme == "google":
                return delta.to_google_format()
            else:
                # This should never happen due to validation in __init__
    ...
    def process_chunk(
        ...
        if "choices" in canonical and canonical["choices"]:
            ...
            if finish_reason == "tool_calls" and not self.state.finished_tool_calls:
                ...
                if "usage" in canonical and canonical["usage"]:
                    self.state.update_usage(canonical["usage"])
                # Mark this as a STOP event to indicate final response
                return (StreamEventType.STOP, canonical)
    ...
    def process_stream(
        ...
        Args:
            response: HTTP response with streaming content or generator yielding lines
        ...
        """
        # Handle different response types - could be aiohttp response or generator
        if hasattr(response, "iter_lines"):
            # aiohttp.ClientResponse
            line_iterator = response.iter_lines(decode_unicode=True)
        elif hasattr(response, "__iter__"):
            # Generator or iterable
            line_iterator = response
        else:
            # Try to iterate directly
            line_iterator = iter(response)
        ...
        for line in line_iterator:
            ...
            if event_type == StreamEventType.DONE:
                # Emit final usage before [DONE] if we have it
                if self.state.usage:
                ...
                # Don't emit [DONE] here - only emit it when we see finish_reason: "stop"
                # This is critical for agentic loops where [DONE] events occur after tool calls
                break
            ...
            elif event_type == StreamEventType.STOP:
                # This is content with finish_reason: "stop" - the final response
                target_chunk = self.convert_to_target_format(canonical_chunk)
                yield f"data: {json.dumps(target_chunk)}\n\n", canonical_chunk

                # Now emit [DONE] as this is truly the end of the stream
                yield "data: [DONE]\n\n", {"type": "done"}
                break
        ...
        # Final usage is now emitted before [DONE] in the loop above
        # This ensures [DONE] is truly the last event emitted
    ...
    def collect(self, response, request_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        Args:
            response: HTTP response with streaming content or generator
...
def unified_stream_adapter(
    ...
    Args:
        ...
        target_scheme: The target provider scheme ('openai', 'anthropic', 'google')
        relay_tool_use_events: Whether to relay tool use events immediately
        relay_usage_events: Whether to relay usage events immediately
        emit_function_announcements: Whether to emit function announcement events
...
# Compatibility function for MemoryProxy migration
def stream_with_buffered_functions(
    ...
    """
    Compatibility wrapper around UnifiedStreamAdapter for MemoryProxy integration.

    This maintains the exact interface expected by MemoryProxy while providing
    the enhanced tool call buffering and format conversion capabilities.

    Args:
        response: HTTP response with streaming content
        source_scheme: The source provider scheme
        target_scheme: The target provider scheme
        relay_tool_use_events: Whether to relay tool use events immediately
        relay_usage_events: Whether to relay usage events immediately
    ...
    yield from adapter.process_stream(
        ...
        emit_function_announcements=True,  # Enable function announcements for better UX
    )
